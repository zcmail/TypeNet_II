...
def create_base_network(dim):
    ...
    Dens_layer_out = Dense(128, activation= 'relu', input_dim= 128, use_bias= True, name='Dense')(lstm_layer1)
...
Dens_layer1 = Dense(128, activation= 'relu', input_dim= 128, use_bias= True, name='Dense1')(abs_of_sub)
Dropout(Dens_layer1, 0.5)
Dens_layer = Dense(1, activation= 'sigmoid', input_dim= 128, use_bias= True, name='Dense')(Dens_layer1)


#model = Model([input_left, input_right], distance)
model = Model([input_left, input_right], Dens_layer)
model.load_weights('./128D_Dense128_1LSTM_distance_step70_epochs_4th100batch_size_512_lr000001_Drop0.5_new.h5')
#model = multi_gpu_model(model,gpus =3)   #用gpu训练

model.summary()

model_path = "./128D_Dense128_1LSTM_distance_step70_epochs_5th"+str(epochs)+"batch_size_"+str(batch_size)+"_lr00000001_Drop0.5_new.h5"
